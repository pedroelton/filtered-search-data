...
function TagsSearch({ data, placeholder }) {
  ...
  const handleFilter = (event) => {
    ...
    //Eh bom filtrar as tags que já foram adicionadas pro cara n colocar duas vezes
    const kkk = data.filter((e) => !tags.includes(e.name));
    const newFilter = kkk.filter((value) => {
  ...
  return (
    <div className="Apps w-screen h-screen flex justify-center items-start flex-col p-5 bg-purple-50">
      ...
      <div className="flex flex-wrap justify-start items-start gap-2">
        {/* Cuidado com as keys do React isso pode dar um bucho no futuro se ficar duplicada*/}
        {tags.map((tag, index) => (
